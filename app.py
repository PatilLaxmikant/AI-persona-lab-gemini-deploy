...
def render_model_settings():
    ...
    with st.sidebar:
        with st.expander("⚙️ Default Model Settings", expanded=False):
            available_models = st.session_state.persona_manager.get_available_models()
            if not available_models:
                st.error("No Google models available.")
                return
...
def render_persona_edit_dialog(persona):
    ...
    This version includes the fix for saving model settings.
    """
    with st.form(f"edit_persona_{persona.id}"):
        ...
        # Define variables *before* the if/else block
        new_model = persona.model
        new_temperature = persona.temperature
        new_max_tokens = persona.max_tokens
